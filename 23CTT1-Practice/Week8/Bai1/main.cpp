...
List GenerateList(const int& slMin, const int& slMax, const int& gtMin, const int& gtMax)
{
	...
	for (int i = 0; i < sl; ++i)
	{
		AddHead(l, gtMin + rand() % (gtMax - gtMin + 1));
	}
...
int main() {
	...
	List l;
	l = GenerateList(5, 10, 0, 100);
	
	cout << "Danh sach ban dau:\n";
	PrintList(l);

	Node* tmp = GetNodePointer1(l, 2);
	cout << "\nNode co chi so 2: " << tmp->info << "\n";

	Node* chen = new Node;
	
	chen = AddBefore(l, l.pHead, 13);
	cout << "Them truoc node dau tien:\n";
	PrintList(l);
	
	AddTail(l, 13);
	cout << "\nThem 13 vao cuoi danh sach:\n";
	PrintList(l);

	int xoadau = RemoveHead(l);
	cout << "\nDanh sach sau khi huy node dau tien:\n";
