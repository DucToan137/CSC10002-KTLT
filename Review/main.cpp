...
void Init(ListGheNgoi* l) {
	l->pHead = l->pTail = NULL;

}

void AddTail(ListGheNgoi *l, GheNgoi x) {
	Node* p = new Node;
	p->data = x;
	p->pNext = NULL;
...
void XuatSinhVien(ListGheNgoi* lrs) {
	Node* p = lrs->pHead;
	while (p != NULL) {
...
ListGheNgoi* LoadData(const char* path) {
	int n, m;
	ListGheNgoi* lgn = new ListGheNgoi;
	Init(lgn);
...
void SapXepListGheNgoi(ListGheNgoi* lgn, int (*sosanh)(GheNgoi, GheNgoi)) {
	Node* p = lgn->pHead;
	while (p != NULL) {
		Node* q = p->pNext;
		while (q != NULL) {
...
//ListGheNgoi* TimDeQuyTheoHo(ListGheNgoi *lgn, const char* ho) {
//	ListGheNgoi* l = new ListGheNgoi;
//	Init(l);
//
//	Node* p = lgn->pHead;
//	while (p != NULL) {
...
void TimDeQuyTheoHoHelper(Node* p, ListGheNgoi* l, const char* ho) {
	if (p == NULL) {
	...
	TimDeQuyTheoHoHelper(p->pNext, l, ho); // Đệ quy đến nút tiếp theo
}
...
ListGheNgoi* TimDeQuyTheoHo(ListGheNgoi* lgn, const char* ho) {
	...
	TimDeQuyTheoHoHelper(lgn->pHead, l, ho); // Bắt đầu đệ quy từ nút đầu tiên
...
ListGheNgoi *TimTheoHoKhuDeQuy(ListGheNgoi* lgn, const char* ho) 
{
	ListGheNgoi* l = new ListGheNgoi;
	Init(l);
	Node* p = lgn->pHead;
...
int main() {
	...
	ListGheNgoi* l = TimDeQuyTheoHo(lgn, "Nguyen");
